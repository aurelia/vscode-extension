...
In previous versions of HTML, the different character encodings could be delimited by spaces or commas. In HTML5, only spaces are allowed as delimiters.`));
    this.attributes.set('action',
      new Attribute(`The URI of a program that processes the form information. This value can be overridden by a formaction attribute on a <button> or <input> element.`));
    this.attributes.set('autocomplete',
      new Attribute(`Indicates whether input elements can by default have their values automatically completed by the browser. This setting can be overridden by an autocomplete attribute on an element belonging to the form.`,
      null,
      null,
      null,
      null,
    ...
    this.attributes.set('enctype',
      new Attribute(`When the value of the method attribute is post, enctype is the MIME type of content that is used to submit the form to the server.`,
      null,
      null,
      null,
      null,
    ...
    this.attributes.set('method',
      new Attribute(`The HTTP method that the browser uses to submit the form.`,
      null,
      null,
      null,
      null,
    ...
    this.attributes.set('name',
      new Attribute(`The name of the form. In HTML 4, its use is deprecated (id should be used instead). It must be unique among the forms in a document and not just an empty string in HTML 5.`));
    this.attributes.set('novalidate',
      new Attribute(`This Boolean attribute indicates that the form is not to be validated when submitted. If this attribute is not specified (and therefore the form is validated), this default setting can be overridden by a formnovalidate attribute on a <button> or <input> element belonging to the form.`));                  
    this.attributes.set('target',
      new Attribute(`A name or keyword indicating where to display the response that is received after submitting the form. In HTML 4, this is the name/keyword for a frame. In HTML5, it is a name/keyword for a browsing context (for example, tab, window, or inline frame).`,
      null,
      null,
      null,
      null,
