...
export default class ImgElement extends MozDocElement {
  ...
  constructor() {
    ...
    this.attributes.set('ismap',
      new BindableAttribute(`This Boolean attribute indicates that the image is part of a server-side map. If so, the precise coordinates of a click are sent to the server.`));
    this.attributes.set('longdesc',
      new BindableAttribute(`A link to a more detailed description of the image. Possible values are a URL or an element id.`));
    this.attributes.set('sizes',
      new BindableAttribute(`A list of one or more strings separated by commas indicating a set of source sizes.`));
    this.attributes.set('src',
      new BindableAttribute(`The image URL. This attribute is mandatory for the <img> element. On browsers supporting srcset, src is treated like a candidate image with a pixel density descriptor 1x unless an image with this pixel density descriptor is already defined in srcset, or unless srcset contains 'w' descriptors.`));
