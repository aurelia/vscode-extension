...
export default class LinkElement extends BaseElement {
  ...
  constructor() {
    ...
    this.attributes.set('href',
      new Attribute(`This attribute specifies the URL of the linked resource. A URL might be absolute or relative.`));
    this.attributes.set('hreflang',
      new Attribute(`This attribute indicates the language of the linked resource. It is purely advisory. Allowed values are determined by BCP47. Use this attribute only if the href attribute is present.`));
    this.attributes.set('media',
      new Attribute(`This attribute specifies the media which the linked resource applies to. Its value must be a media query. This attribute is mainly useful when linking to external stylesheets by allowing the user agent to pick the best adapted one for the device it runs on.`));
    this.attributes.set('rel',
      new Attribute(`This attribute names a relationship of the linked document to the current document. The attribute must be a space-separated list of the link types values. The most common use of this attribute is to specify a link to an external style sheet: the rel attribute is set to stylesheet, and the href attribute is set to the URL of an external style sheet to format the page. WebTV also supports the use of the value next for rel to preload the next page in a document series.`));
    this.attributes.set('sizes',
      new Attribute(`This attribute defines the sizes of the icons for visual media contained in the resource. It must be present only if the rel contains the icon link types value.`));
