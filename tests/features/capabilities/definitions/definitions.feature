Feature: Definition in View
  ...
  # | Bindable           | 20   | `            \|inter-bindable.bind="increaseCounter()"` | my-compo.ts      |
  # | Bindable           | 21   | `            \|string-bindable.bind="grammarRules"`     | my-compo.ts      |
  # | Text interpolation | 26   | `  ${} ${\|grammarRules.length}`                        | compo-user.ts    |
  # | DESCRIPTION        | 29   | `    <label repeat.for="\|rule of grammarRules">`       | compo-user.ts    |
  # | DESCRIPTION        | 29   | `    <label repeat.for="rule of \|grammarRules">`       | compo-user.ts    |
